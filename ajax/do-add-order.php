...
require_once __DIR__ . "/../include/header-functions.php";
//$data = array();
...
$data =array("user_id"=>"{$_SESSION['user_id']}","date"=>"{$date}","room_number"=>"{$room_number}","status"=>"processing","nodes"=>"{$_POST['nodes']}");
...
foreach ($_POST['orders'] as $key=>$value ){  
  $new=array("user_id"=>"{$_SESSION['user_id']}","product_id"=>"{$value->id_prod}","order_id"=>"{$id}","product_count"=>"{$value->count}","product_price"=>"{$value->price}");
